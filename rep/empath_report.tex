...
for train_idx, test_idx in logo.split(X_bio, y, groups):
    ...
    # Person-specific normalization (Snippet 2)
    ...
    rf_bio = RandomForestClassifier(
        ...
        random_state=42).fit(Xb_tr, y_tr)
    rf_lm = RandomForestClassifier(
        n_estimators=300, max_depth=4,
        min_samples_split=10, max_features='sqrt',
        random_state=42).fit(Xl_tr, y_tr)

    # Stacking: concatenate probability outputs
    X_meta_tr = np.hstack([
        rf_bio.predict_proba(Xb_tr),
        rf_lm.predict_proba(Xl_tr)])
    X_meta_te = np.hstack([
        rf_bio.predict_proba(Xb_te),
